...
Originally built to stabilize NextCloud running on a Raspberry Pi 2 (an old, underpowered board) behind Traefik. NextCloud's web UI fires many concurrent requests, which overwhelmed the Pi and caused crashes. This middleware queues the excess requests and lets them through gradually, rather than hammering the backend all at once.
...
Set these options wherever you declare Traefik middlewares — the file provider, Docker labels, and so on. See [Working with Traefik Plugins](https://plugins.traefik.io/install) for how to declare and configure a plugin middleware.
...
The right values depend on **your** service and its traffic — the defaults suit a small, underpowered homelab app, but a busier or beefier backend will want different numbers. Watch how the app behaves under load and adjust:
